...
class AlignedCursor(object):
    ...
    def read_bool(self, length):
        ...
        if bool_int == 0:
            ...
            raise AssertionError('Expected Bool, but not 0 or 1')
...
def main():
    ...
    with open(args.filename, 'rb') as save_file:
        content_bytes = save_file.read()


    cursor = Cursor(content_bytes, index=0)
    header_magic_string = cursor.read_bytes(4)
    header_version = cursor.read_int(4)  
    header_size = cursor.read_int(4)
    thumbnail_offset_lower_dword = cursor.read_int(4)
    thumbnail_offset_upper_dword = cursor.read_int(4)
    thumbnail_size = cursor.read_int(4)
    guid_game_id = cursor.read_bytes(16)
    game_name = cursor.read_array_of_ints(
        element_size=2,
        array_size=1024)
    save_game_name = cursor.read_array_of_ints(
        element_size=2,
        array_size=1024)
    level_name = cursor.read_array_of_ints(
        element_size=2,
        array_size=1024)
    comments = cursor.read_array_of_ints(
        element_size=2,
        array_size=1024)
    savegame_sig = cursor.read_bytes(32)
    description = cursor.read_string(180)
    savegame_version = cursor.read_int(4)
    is_screenshot = cursor.read_int(4)
    if is_screenshot:
        raise NotImplementedError('No support for saved games with screenshots')
    version_string = cursor.read_string(200) 
    main_data_filename = cursor.read_string(180)
    screenheight = cursor.read_int(4)
    colordepth = cursor.read_int(4)
    gamespeed = cursor.read_int(4)
    mode = cursor.read_int(4)
    savecursor = cursor.read_int(4)
    offset_x = cursor.read_int(4)
    offset_y = cursor.read_int(4)
    loop_counter = cursor.read_int(4)
    spriteset_size = cursor.read_int(4)
    sprite_count = cursor.read_int(4)

    if sprite_count > 0:
        raise NotImplementedError('No support for serialized sprites, yet')

    global_data_size = cursor.read_int(4)
    global_data = cursor.read_bytes(global_data_size)
    num_script_modules = cursor.read_int(4)
    ...
    current_room = cursor.read_int(4)

    rooms = read_room_states(cursor)
    print('\n\n')
    game_state = read_game_state(cursor)
    if game_state['num_do_once_tokens'] > 0:
        raise NotImplementedError('do once token logic not implemented')
    guess_numgui = get_numgui(cursor)
    gui_draw_order = cursor.read_array_of_ints(
        element_size=4,
    ...
    guess_num_characters = total_runs - MAX_INIT_SPR
    print("I'm guessing the value of num_characters should be {}".format(
        guess_num_characters))

    game_struct_base = read_game_struct_base(cursor)

    assert (
        game_struct_base['numcharacters'] == guess_num_characters,
    ...
    game_struct = read_game_struct(cursor, game_struct_base)
    palette = [x.hex() for x in cursor.read_array(
        element_size=COLOR_BYTES,
        array_size=256)]
    dialogs = read_dialogs(cursor, game_struct_base['numdialog'])
    more_dynamic_values = read_more_dynamic_values(cursor)
    gui = read_gui(cursor)
    audiocliptypes = read_audio_clip_types(cursor)
    this_room = read_this_room(cursor)
    ambient_sounds = read_ambient_sounds(cursor)
    overlays = read_overlays(cursor)
    dynamic_surfaces = read_dynamic_surfaces(cursor)
    displayed_room_status = read_displayed_room_status(
        cursor,
        game_state)
    global_vars = read_global_vars(cursor)
    game_views = read_game_views(cursor)
    audioclips = read_audioclips(cursor)
    plugin_data = read_plugin_data(cursor)
    this_room_volume = cursor.read_int(4)
    deserialized_objects = read_serialized_objects(cursor)
    current_music_type = cursor.read_int(4)
...
def read_overlays(cursor):
    ...
    align_cursor = AlignedCursor(cursor)
...
def read_move_lists(cursor):
    ...
    while True:
        ...
        try:
            move_list = read_move_list(align_cursor)
...
def read_move_list(align_cursor):
    move_list = {}
...
def read_room_state(cursor, been_here=None):
    room = {}
    room_align_cursor = AlignedCursor(cursor)
    ...
    if been_here is None or been_here:
        ...
        for j in range(MAX_INIT_SPR):
            object_align_cursor = AlignedCursor(
                room_align_cursor)
